-- Daily profit should follow the shared 08:00 Istanbul game-day boundary.
-- Persist the last day anchor so daily profit resets correctly after relaunch/cloud load.

alter table public.player_stats
  add column if not exists profit_day_anchor_at timestamptz null;

update public.player_stats
set profit_day_anchor_at = coalesce(profit_day_anchor_at, now())
where profit_day_anchor_at is null;
