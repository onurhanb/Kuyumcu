alter table public.player_stats
  add column if not exists tax_debt double precision not null default 0,
  add column if not exists last_tax_charged_day integer not null default 0;

update public.player_stats
set
  tax_debt = coalesce(tax_debt, 0),
  last_tax_charged_day = coalesce(last_tax_charged_day, 0)
where tax_debt is null
   or last_tax_charged_day is null;
