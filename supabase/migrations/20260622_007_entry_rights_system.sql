-- v1.3 entry rights system rollout
-- Keeps legacy reputation columns physically present for rollout safety,
-- but app/runtime/save flow stops using them.

alter table public.player_stats
  add column if not exists entry_rights_remaining integer not null default 3,
  add column if not exists entry_rights_refreshed_at timestamptz null;

update public.player_stats
set
  entry_rights_remaining = 3,
  entry_rights_refreshed_at = coalesce(entry_rights_refreshed_at, now())
where entry_rights_remaining is distinct from 3
   or entry_rights_refreshed_at is null;
